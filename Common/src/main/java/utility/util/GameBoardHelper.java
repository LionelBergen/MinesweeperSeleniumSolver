...
public final class GameBoardHelper<T extends GameSquare> {
	...
	public List<T> getSurroundingFlaggedSquares(List<T> gameSquares, GameSquare square) {
		return getAllSquaresOfType(gameSquares, SquareValue.FLAGGED).stream().filter(e-> isTouching(square, e)).collect(Collectors.toList());
	}
	
	public List<T> getSurroundingBlankSquares(List<T> gameSquares, GameSquare square) {
	 ...
	 */
	public List<T> getAllBlankSquares(List<T> gameSquares) {
	...
	private List<T> getAllSquaresOfType(List<T> gameSquares, SquareValue squareValueToFilterBy) {
